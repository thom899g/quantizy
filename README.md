...
**Fit a bigger brain on your Mac, with receipts.**
...
**V1 personal license: $49.**

Quantizy V1 is a signed, notarized Apple Silicon app for current-RAM fit
checks, memory guardrails, pressure-aware recipes, and validated artifact
handoff. It is a practical local-AI utility, not a promise that every giant
...
After a confirmed GitHub Sponsors payment, open one license request from the
same GitHub account. Manual delivery sends the offline license key, exact V1
download link, SHA-256 checksum, buyer guide, and support path. Automatic Stripe
checkout is intentionally closed until the production worker passes its release
readiness gate.

## V1 Status

**The V1 artifact is shipped on GitHub Releases.** The DMG is Apple Silicon,
Developer ID signed, Apple-notarized, stapled, and checksum-matched to the
release manifest. The local payment-to-delivery smoke test passes, while live
Stripe fulfillment remains gated on an external worker that is not currently
resolving.

V1's honest value is helping a normal Mac user answer: “Can I run this local
model with the RAM I have free right now, while my IDEs and other apps are
open?” It can recommend a safer context, quantization, KV, paging, and prefill
recipe, or clearly say to choose a smaller model or close memory-heavy apps.

It does **not** claim a proven universal quality win over Unsloth, GGUF dynamic
quants, or the right uniform bit-width. That research gate remains open.

The versioned core notes below are historical engineering notes. The current
customer artifact, price, release URL, and checksum are the V1 details above.

## DeepSeek-Informed Core

The engine carries receipt-gated planning paths inspired by real research:
latent KV compression from [DeepSeek-V2](https://arxiv.org/abs/2405.04434),
sparse/indexed attention ideas from [DeepSeek-V3.2-Exp](https://api-docs.deepseek.com/news/news250929),
and long-context sparse-KV directions discussed for [DeepSeek-V4](https://huggingface.co/blog/deepseekv4).
...
> The checksum is the V1 GitHub Release asset. Automatic Stripe checkout must
> stay closed until the worker readiness endpoint and hosted handoff both pass.
...
The compression-quality claim is not being marketed as a breakthrough. The
earlier OLMoE and Granite receipts are research evidence only; broader models
and strong dynamic-quant baselines still need independent validation.
...
Automatic Stripe checkout opens only after the worker and public handoff checks
are green.
